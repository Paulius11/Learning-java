...
public class StreamPraktiniai {
	...
	public static List<Integer> getAgeFromUsers(List<User> users) {
		return users.stream().map(x -> x.getAge()).collect(Collectors.toList());
	}
...
//        return integers.stream().reduce(0, (x, y) -> x + y);
	...
	public static List<String> getDistinctLetters(List<String> names) {
		return names.stream().map(x -> x).map(v -> v.split("")).flatMap(Arrays::stream)
//        		.peek(System.out::println)
	...
	public static Integer getMaxAge(List<User> users) {
		return users.stream().mapToInt(x -> x.getAge()).max().orElse(0);
	}

	public static Integer getMinAge(List<User> users) {
		return users.stream().mapToInt(x -> x.getAge()).min().orElse(0);
	}
	...
	public static List<Integer> generateFirst10PrimeNumbers() {
		return Stream.iterate(2, n -> n + 1).filter(x -> isPrime(x)).limit(10).collect(Collectors.toList());
	}
